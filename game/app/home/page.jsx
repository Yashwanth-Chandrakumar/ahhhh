...
export default function ChickenGamePage() {
  ...
  const initAudio = useCallback(async () => {
    if (audioContextRef.current && audioContextRef.current.state !== 'closed') {
      ...
      // If it exists and is suspended, it will be resumed by ensureAudioContextRunning
      return;
    }
    ...
    // Do not set gameState to 'ready' here directly, let the calling flow manage it
    // or the main useEffect.
  }, [setIsMicrophoneAllowed]); // Removed gameState dependency

  const ensureAudioContextRunning = useCallback(async () => {
    if (!audioContextRef.current || audioContextRef.current.state === 'closed') {
      console.log("AudioContext not initialized or closed, attempting to re-initialize.");
      await initAudio(); // Try to set it up if it's missing
    }
    ...
    if (audioContextRef.current) {
      if (audioContextRef.current.state === 'suspended') {
        ...
        return false; // Or attempt init/resume again depending on state
      }
    }
    console.error("AudioContext could not be initialized or started.");
    setIsMicrophoneAllowed(false); // If it's still not there after init attempt
    return false;
  ...
  const generateLevel = useCallback(() => { /* ... same ... */
  ...
  const getSoundInfo = useCallback(() => {
    // Log AudioContext state frequently if debugging
    // if (audioContextRef.current) { console.log("getSoundInfo - AudioContext state:", audioContextRef.current.state); }
    ...
    if (!analyserRef.current || !dataArrayRef.current || dataArrayRef.current.length === 0 ||
      ...
      // If context isn't running, sound info is effectively zero.
  ...
  const resetGame = useCallback(() => { /* ... same ... */
    chickenRef.current = { worldX: CHICKEN_INITIAL_WORLD_X, y: CANVAS_HEIGHT - 100 - CHICKEN_HEIGHT, vy: 0, width: CHICKEN_WIDTH, height: CHICKEN_HEIGHT, onGround: true, bobOffset: 0, bobDirection: 1, };
  ...
  const startGame = useCallback(async () => {
    ...
    if (!audioReady) {
      ...
      // Alert or message to user already handled in ensureAudioContextRunning
      return;
    }
  ...
  const updateGame = useCallback(() => {
    ...
    levelElementsRef.current.forEach(element => { const elementScreenX = element.x - cameraXRef.current;
      if (element.type === 'platform' || element.type === 'bridge_post' || element.type === 'bridge_plank_active') { const elRect = { x: elementScreenX, y: element.y, width: element.width, height: element.height }; if (chickenRect.x + chickenRect.width > elRect.x && chickenRect.x < elRect.x + elRect.width && chickenRect.y + chickenRect.height > elRect.y && chickenRect.y + chickenRect.height < elRect.y + Math.max(chicken.vy,0) + 20 && chicken.vy >= 0) { chicken.y = elRect.y - chickenRect.height; chicken.vy = 0; chicken.onGround = true; onAnySurface = true; if (element.type === 'bridge_post') bridgeStateRef.current.playerOnBridgePost = true; else bridgeStateRef.current.playerOnBridgePost = false; }
        if (element.hasSpike) { const spikeRect = { x: elRect.x + element.spikeRelativeX, y: elRect.y - 30, width: 30, height: 30 }; if (chickenRect.x < spikeRect.x + spikeRect.width && chickenRect.x + chickenRect.width > spikeRect.x && chickenRect.y < spikeRect.y + spikeRect.height && chickenRect.y + chickenRect.height > spikeRect.y) { setGameState('gameOver'); } }
      } else if (element.type === 'shuriken_spawn' && !element.spawned) { if (element.x - cameraXRef.current < CANVAS_WIDTH + 100 && element.x - cameraXRef.current > -100) { levelElementsRef.current.push({ type: 'shuriken_active', id: `sa-${element.id}-${Date.now()}`, worldX: element.x + Math.random() * 100 - 50, y: Math.random() * (CANVAS_HEIGHT / 2) + 50, size: 30, speedX_world: -(WALK_SPEED + 2 + Math.random() * 3), rotation: 0, active: true, }); element.spawned = true; }
      } else if (element.type === 'shuriken_active') { element.worldX += element.speedX_world; const shurikenScreenX = element.worldX - cameraXRef.current; element.rotation += 0.2; if (shurikenScreenX + element.size < -50) element.active = false; const shurikenRect = { x: shurikenScreenX, y: element.y, width: element.size, height: element.size }; if (chickenRect.x < shurikenRect.x + shurikenRect.width && chickenRect.x + chickenRect.width > shurikenRect.x && chickenRect.y < shurikenRect.y + shurikenRect.height && chickenRect.y + chickenRect.height > shurikenRect.y) { setGameState('gameOver'); }
      } else if (element.type === 'bridge_structure') { if (bridgeStateRef.current.playerOnBridgePost && element.activePlanks < element.planks.length) { const soundForBridge = getSoundInfo(); if (soundForBridge.detected || chicken.onGround) { const newPlankIndex = element.activePlanks; const plankDef = element.planks[newPlankIndex]; const plankId = `bpa-${element.id}-${newPlankIndex}`; if (!levelElementsRef.current.find(e => e.id === plankId)) { levelElementsRef.current.push({ type: 'bridge_plank_active', id: plankId, x: element.x + plankDef.relativeX, y: element.y + plankDef.yOffset, width: plankDef.width, height: plankDef.height, }); element.activePlanks++; } } }
      } else if (element.type === 'finish_line') { const finishRect = { x: elementScreenX, y: element.y, width: element.width, height: element.height }; if (chickenRect.x + chickenRect.width > finishRect.x && chickenRect.x < finishRect.x + finishRect.width && chickenRect.y + chickenRect.height > finishRect.y && chickenRect.y < finishRect.y + finishRect.height) { setGameState('won'); spawnConfetti(); } }
    });
    ...
    if (gameState === 'playing') { setScore(chicken.worldX / 100); }
  ...
  const refinedDraw = useCallback(() => { /* ... same draw logic using screenX ... */
    const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); if (!ctx) return;
    ctx.fillStyle = COLOR_SKY; ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    ctx.fillStyle = COLOR_WATER; ctx.fillRect(0, CANVAS_HEIGHT - 50, CANVAS_WIDTH, 50); ctx.fillStyle = COLOR_WAVE;
    ...
    levelElementsRef.current.forEach(element => { const elementScreenX = element.x - cameraXRef.current; if (elementScreenX + (element.width || element.size || 50) < 0 || elementScreenX > CANVAS_WIDTH) { return; }
      if (element.type === 'platform' || element.type === 'bridge_post' || element.type === 'bridge_plank_active') { ctx.fillStyle = COLOR_PLATFORM_DIRT; ctx.fillRect(elementScreenX, element.y, element.width, element.height); if (element.type === 'platform') { ctx.fillStyle = COLOR_PLATFORM_GRASS; ctx.fillRect(elementScreenX, element.y, element.width, 20); } else if (element.type === 'bridge_plank_active') { ctx.fillStyle = COLOR_BRIDGE_PLANK; ctx.fillRect(elementScreenX, element.y, element.width, element.height); } if (element.hasSpike) { const spikeWidth = 30, spikeHeight = 30; const spikeX = elementScreenX + element.spikeRelativeX; const spikeY = element.y - spikeHeight; ctx.fillStyle = COLOR_SPIKE; ctx.beginPath(); ctx.moveTo(spikeX, spikeY + spikeHeight); ctx.lineTo(spikeX + spikeWidth / 2, spikeY); ctx.lineTo(spikeX + spikeWidth, spikeY + spikeHeight); ctx.closePath(); ctx.fill(); }
      } else if (element.type === 'shuriken_active') { const shurikenScreenX = element.worldX - cameraXRef.current; if (shurikenScreenX + element.size < 0 || shurikenScreenX > CANVAS_WIDTH) return; ctx.save(); ctx.translate(shurikenScreenX + element.size / 2, element.y + element.size / 2); ctx.rotate(element.rotation); ctx.fillStyle = '#555555'; ctx.strokeStyle = '#333333'; ctx.lineWidth = 2; const armLength = element.size / 2; ctx.beginPath(); for (let j = 0; j < 4; j++) { ctx.moveTo(0,0); ctx.lineTo(armLength * Math.cos(Math.PI/2 * j), armLength * Math.sin(Math.PI/2*j)); ctx.lineTo(armLength/2 * Math.cos(Math.PI/2 * j + Math.PI/4), armLength/2 * Math.sin(Math.PI/2*j + Math.PI/4)); } ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.restore();
      } else if (element.type === 'warning_sign') { const signSize = 50; ctx.fillStyle = '#FFCC00'; ctx.beginPath(); ctx.moveTo(elementScreenX + signSize / 2, element.y); ctx.lineTo(elementScreenX + signSize, element.y + signSize * 0.866); ctx.lineTo(elementScreenX, element.y + signSize * 0.866); ctx.closePath(); ctx.fill(); ctx.strokeStyle = '#000000'; ctx.lineWidth = 3; ctx.stroke(); ctx.fillStyle = '#000000'; ctx.font = 'bold 30px Arial'; ctx.textAlign = 'center'; ctx.fillText('!', elementScreenX + signSize / 2, element.y + signSize * 0.65);
      } else if (element.type === 'finish_line') { const poleWidth = 10; ctx.fillStyle = '#777777'; ctx.fillRect(elementScreenX, element.y, poleWidth, element.height); const squareSize = 10; for (let r = 0; r < 3; r++) { for (let c = 0; c < 5; c++) { ctx.fillStyle = (r + c) % 2 === 0 ? FINISH_LINE_COLOR_PRIMARY : FINISH_LINE_COLOR_SECONDARY; ctx.fillRect(elementScreenX + poleWidth + c * squareSize, element.y + r * squareSize, squareSize, squareSize); } } }
    });
  ...
  useEffect(() => {
    generateLevel();
    if (isMicrophoneAllowed === null) { // Only try to init if we haven't determined permission yet
      initAudio().then(() => {
          // After attempting init, if we are still loading, move to ready.
          // This handles the case where initAudio might complete after initial render.
          if(gameState === 'loading') {
              setGameState('ready');
          }
      });
    } else if (gameState === 'loading') { // If mic status known but still loading
        setGameState('ready');
    }
    ...
    const handleKeyPress = (e) => {
      if (e.code === 'Space') {
        if (gameState === 'playing' && chickenRef.current.onGround) {
          ...
          startGame(); // startGame will handle audio context resume
        }
      }
    };
  ...
  }, [generateLevel, initAudio]); // Removed startGame, gameState, isMicrophoneAllowed from deps for this specific setup effect.
                                  // initAudio is stable due to its own useCallback. generateLevel too.
  ...
  }, [gameState, updateGame, refinedDraw]); // updateGame and refinedDraw are memoized
